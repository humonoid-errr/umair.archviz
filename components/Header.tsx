...
const Header: React.FC<HeaderProps> = ({ onNavigate, page, projects, onSelectProject, isZenMode = false }) => {
  ...
  const isLightPage = !isHomePage && page !== 'gallery';
  ...
  const getCapsuleClasses = () => {
    ...
    if (isWorkMenuOpen) return `${baseClasses} ${positionClasses} opacity-100 scale-x-100 bg-gray-200 border border-gray-300`;
    return `${baseClasses} ${positionClasses} ${hoverClasses} bg-gray-100 border border-gray-200`;
  };

  const getTextClasses = () => isLightPage ? 'text-gray-800' : 'text-white';
  const getButtonClasses = () => isLightPage 
    ? "cursor-pointer hover:opacity-70 transition-opacity bg-transparent border-none text-gray-800 uppercase text-xs font-light tracking-widest"
    : "cursor-pointer hover:opacity-70 transition-opacity bg-transparent border-none text-white uppercase text-xs font-light tracking-widest";
      ...
      {isMobileMenuOpen && (
        <div className={`fixed inset-0 bg-white z-[60] text-gray-900 p-8 flex flex-col transform transition-transform duration-300 ease-in-out ${isMenuVisible ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="flex justify-end">
            <button onClick={closeMobileMenu} className="p-2 -m-2"><CloseIcon className="w-8 h-8" /></button>
          </div>
          <nav className="flex-grow flex flex-col pt-8 space-y-12">
            <div>
              <div className="text-3xl font-light mb-4"><span className="font-thin mr-2">+</span> Work</div>
              <ul className="pl-2 space-y-2">
                {projects.map(project => (
                  <li key={project.id}>
                    <button onClick={() => handleMobileProjectSelect(project)} className="w-full text-left py-1 text-xl font-light text-gray-500">{project.name}</button>
                  </li>
                ))}
              </ul>
            </div>
            <div className="flex flex-col space-y-6">
              <button onClick={() => handleMobileNav('about')} className="text-3xl font-light text-left">About</button>
              <button onClick={() => handleMobileNav('services')} className="text-3xl font-light text-left">Services</button>
              <button onClick={() => handleMobileNav('testimonials')} className="text-3xl font-light text-left">Testimonials</button>
              <button onClick={() => handleMobileNav('contact')} className="text-3xl font-light text-left">Contact</button>
            </div>
          </nav>
        </div>
      )}
