...
const Header: React.FC<HeaderProps> = ({ onNavigate, page, projects, onSelectProject }) => {
  ...
  const isLightPage = page === 'about' || page === 'contact' || page === 'gallery';
  ...
  return (
    <>
            ...
            <nav className="flex items-center space-x-6 text-sm font-light tracking-wider uppercase">
              ...
              >
                ...
                {isWorkMenuOpen && projects.length > 0 && (
                  <div className="absolute top-full pt-4 -ml-4 w-auto text-white z-30">
                    <div className="bg-black/30 backdrop-blur-lg border border-white/10 rounded-md overflow-hidden flex flex-col">
                        ...
                        {projects.map(project => (
                          <li key={project.id}>
                            <button 
                              ...
                              className="w-full text-left px-6 py-2 text-sm font-light hover:bg-white/10 transition-colors whitespace-nowrap"
                            >
                              {project.name}
                            </button>
                          </li>
                      ...
                      <p className="text-xs text-gray-400 font-light tracking-wider text-center py-2 border-t border-white/10">
                        © Mohd Umair 2025
                      </p>
                    </div>
                  </div>
      ...
      {isMobileMenuOpen && (
        <div className={`fixed inset-0 bg-black z-50 text-white p-8 flex flex-col md:hidden transform transition-transform duration-300 ease-in-out ${isMenuVisible ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="flex justify-end">
            <button onClick={closeMobileMenu} aria-label="Close menu" className="p-2 -m-2">
              <CloseIcon className="w-8 h-8 text-white" />
            </button>
          </div>
          <nav className="flex-grow flex flex-col justify-start items-start text-left pt-8 overflow-y-auto space-y-12">
            <div>
              <div className="flex items-center text-3xl font-light mb-4">
                  <span className="text-3xl font-thin mr-2">+</span> Work
              </div>
              <ul className="pl-2 space-y-2 max-h-60 overflow-y-auto [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-gray-800 [&::-webkit-scrollbar-thumb]:bg-gray-600 [&::-webkit-scrollbar-thumb]:rounded-full">
                {projects.map(project => (
                  <li key={project.id}>
                    <button 
                      onClick={() => handleMobileProjectSelect(project)}
                      className="w-full text-left py-1 text-xl font-light text-gray-300 hover:text-white transition-colors"
                    >
                      {project.name}
                    </button>
                  </li>
          ...
          <div className="pt-6 mt-auto">
            <div className="flex justify-start space-x-6 mb-6">
                <a href="https://linkedin.com/in/mumair-" target="_blank" rel="noopener noreferrer" className="text-white hover:opacity-70 transition-opacity">
                    <LinkedinIcon className="w-6 h-6" />
                </a>
                <a href="https://wa.me/919412505677" target="_blank" rel="noopener noreferrer" className="text-white hover:opacity-70 transition-opacity">
                    <WhatsAppIcon className="w-6 h-6" />
                </a>
            </div>
            <div className="text-xs text-gray-400 font-light space-y-2 text-left">
            ...
            <p className="text-xs text-gray-500 font-light mt-8 text-left tracking-wider">
              © Mohd Umair 2025
            </p>
          </div>
        </div>
